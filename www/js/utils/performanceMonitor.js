...
class PerformanceMonitor {
     ...
     */
        ...
        if (metric.duration > 100) {
            console.warn(`[Performance] ${metric.name} took ${metric.duration.toFixed(2)}ms`);
        }
    ...
    logSummary() {
        ...
        byName.forEach((durations, name) => {
            ...
            console.log(`${name}: avg ${avg.toFixed(2)}ms, min ${min.toFixed(2)}ms, max ${max.toFixed(2)}ms (${durations.length} calls)`);
        });
