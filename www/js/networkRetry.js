...
export async function retryWithBackoff(requestFn, options = {}) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            ...
            const customShouldRetry = shouldRetry ? shouldRetry(error, attempt) : true;
            
            if (!isRetryable || !customShouldRetry || attempt === maxRetries) {
                throw error;
            }
            
            console.warn(`[Network] Request failed (attempt ${attempt + 1}/${maxRetries + 1}), retrying in ${delay}ms:`, error.message);
 ...
 */
export async function batchFetchWithRetry(urls, options = {}, retryOptions = {}) {
...
export async function checkOnlineStatus(maxAttempts = 3) {
    for (let i = 0; i < maxAttempts; i++) {
        if (navigator.onLine) {
            ...
            try {
                await fetchWithRetry('https://www.google.com/favicon.ico', {
                    method: 'HEAD',
                    timeout: 5000
                }, {
                    maxRetries: 1,
                ...
                console.warn(`[Network] Connectivity check failed (attempt ${i + 1}/${maxAttempts})`);
            }
        }
...
export class OfflineRequestQueue {
    constructor() {
        ...
        this.storageKey = 'pantryai-offline-queue';
    ...
    async processQueue() {
        ...
        for (let i = 0; i < this.queue.length; i++) {
            ...
            try {
                ...
                request.attempts++;
                console.warn(`[Network] Failed to sync request (attempt ${request.attempts}): ${request.url}`);
