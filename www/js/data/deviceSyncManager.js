...
class DeviceSyncManager {
    ...
    async init() {
        ...
        if (deviceInfo && deviceInfo.value) {
            ...
            console.log('[DeviceSync] Loaded device:', this.deviceId, this.deviceName);
        } else {
    ...
    async registerDevice(customName = null) {
        ...
        console.log('[DeviceSync] Registered new device:', this.deviceId, this.deviceName);
    ...
    async setDeviceName(name) {
        ...
        await this.saveDeviceInfo();
        this.notifyListeners('renamed', { deviceId: this.deviceId, deviceName: name });
        return this.getDeviceInfo();
    }
    ...
    async getDataForSync(dataType) {
        switch (dataType) {
            ...
            case 'groceryPreferences':
                const grocery = await db.get('preferences', 'grocery-delivery-preferences');
    ...
    async mergeData(dataType, incomingData) {
        ...
        if (config.strategy === CONFLICT_STRATEGIES.LAST_WRITE_WINS) {
            if (incomingTime > localTime) {
                ...
                return { action: 'rejected', reason: 'local-newer', strategy: 'last-write-wins' };
            }
        }

        if (config.strategy === CONFLICT_STRATEGIES.MERGE_ARRAYS) {
            const merged = this.mergeArrays(currentData, incomingData.value, config.keyField);
            await this.saveData(dataType, merged);
            return { action: 'merged', strategy: 'merge-arrays', items: merged.length };
        }
        ...
        return { action: 'rejected', reason: 'conflict', strategy: config.strategy };
    }
