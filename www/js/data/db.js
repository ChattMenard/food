...
class PantryDB {
  ...
  async loadRecipes(progressCallback) {
    ...
    for (let i = 0; i < normalized.length; i += chunkSize) {
      ...
      if (progressCallback) {
        progressCallback(
          (i + chunk.length) / normalized.length,
          i + chunk.length
        );
      }
    }
  ...
  normalizeRecipe(recipe) {
    const ingredients = Array.isArray(recipe.ingredients)
      ? recipe.ingredients
      : [];
    const clean = Array.isArray(recipe.ingredients_clean)
  ...
  calculateRelevanceScore(recipe, tokens) {
    let score = 0;
    const text =
      `${recipe.name} ${(recipe.ingredients_clean || []).join(' ')}`.toLowerCase();
  ...
  async getPersonalizedRecommendations(pantryItems, preferences = {}) {
    ...
    const scored = recipes.map((recipe) => {
      ...
      if (preferences.diet && recipe.dietary_flags?.[preferences.diet])
        score *= 1.5;
      if (preferences.cuisine && preferences.cuisine === recipe.cuisine)
        score *= 1.3;

      if (preferences.maxTime) {
        const timeBonus = Math.max(
          0,
          1 - (recipe.minutes - preferences.maxTime) / 120
        );
        score *= 1 + timeBonus;
      }
  ...
  findBestMatch(ingredient, pantryVector) {
    let bestScore = 0;
    const ingWords = String(ingredient)
      .toLowerCase()
      .split(/\W+/)
      .filter(Boolean);
  ...
  async getPreferences() {
    ...
    return (
      prefs || { people: 1, diet: 'none', allergy: 'none', cuisine: 'all' }
    );
  }
  ...
  async cleanupSyncedMutations(olderThanMs = 24 * 60 * 60 * 1000) {
    ...
    const allSynced = await this.getAllFromIndex(
      'queuedMutations',
      'status',
      'synced'
    );
