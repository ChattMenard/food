...
import { getPending, markSynced, markFailed, incrementRetry } from './mutationQueue.js';
...
class SyncProcessor {
    ...
    async processMutation(mutation) {
        const { id, type, payload, entityId, retryCount = 0 } = mutation;

        // Check max retries
        if (retryCount >= MAX_RETRIES) {
            console.log(`[SyncProcessor] Max retries reached for ${id}, marking failed`);
        ...
        if (retryCount > 0) {
            console.log(`[SyncProcessor] Retrying ${id} after ${delay}ms (attempt ${retryCount + 1})`);
