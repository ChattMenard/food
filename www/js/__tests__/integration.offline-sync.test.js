...
describe('Offline → Sync Integration', () => {
  ...
  beforeEach(() => {
    ...
    db.getPendingMutations = jest.fn(async () => {
      return Array.from(mockMutations.values()).filter(
        (m) => m.status === 'pending'
      );
    });
  ...
  describe('Mutation Queue Operations', () => {
    it('should handle multiple enqueue operations', async () => {
      const mutations = await Promise.all([
        enqueue({
          type: 'ADD_ITEM',
          payload: { name: 'item1' },
          entityId: 'pantry:item1',
        }),
        enqueue({
          type: 'ADD_ITEM',
          payload: { name: 'item2' },
          entityId: 'pantry:item2',
        }),
        enqueue({
          type: 'UPDATE_ITEM',
          payload: { name: 'item3' },
          entityId: 'pantry:item3',
        }),
      ]);
    ...
    it('should generate unique IDs for each mutation', async () => {
      const mutation1 = await enqueue({
        type: 'ADD_ITEM',
        payload: { name: 'item1' },
        entityId: 'pantry:item1',
      });
      const mutation2 = await enqueue({
        type: 'ADD_ITEM',
        payload: { name: 'item2' },
        entityId: 'pantry:item2',
      });
