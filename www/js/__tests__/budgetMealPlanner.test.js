...
import { BudgetMealPlanner, BUDGET_TIERS, BUDGET_SUBSTITUTIONS } from '../features/plan/budgetMealPlanner.js';
...
describe('BudgetMealPlanner', () => {
    ...
    describe('initialization', () => {
        ...
        it('should get current tier details', () => {
            ...
            expect(tier.maxPerServing).toBe(6.00);
        });
    });
    ...
    describe('tier management', () => {
        ...
        it('should set tier correctly', async () => {
            ...
            expect(planner.getCurrentTier().maxPerServing).toBe(3.00);
        });

        it('should throw for unknown tier', async () => {
            await expect(planner.setTier('unknown')).rejects.toThrow('Unknown budget tier');
        });
    });
    ...
    describe('budget filtering', () => {
        const mockRecipes = [
            { id: 1, name: 'Rice & Beans', servings: 4, ingredients: [
                { name: 'rice', quantity: 1 },
                { name: 'beans', quantity: 1 }
            ]},
            { id: 2, name: 'Steak Dinner', servings: 2, ingredients: [
                { name: 'beef steak', quantity: 2 },
                { name: 'potatoes', quantity: 2 }
            ]},
            { id: 3, name: 'Salmon', servings: 2, ingredients: [
    ...
    describe('weekly plan generation', () => {
        const affordableRecipes = [
            { id: 1, name: 'Rice & Beans', servings: 4, ingredients: [{ name: 'rice', quantity: 1 }, { name: 'beans', quantity: 1 }] },
            { id: 2, name: 'Egg Fried Rice', servings: 4, ingredients: [{ name: 'rice', quantity: 1 }, { name: 'eggs', quantity: 4 }] },
            { id: 3, name: 'Pasta Aglio', servings: 4, ingredients: [{ name: 'pasta', quantity: 1 }, { name: 'garlic', quantity: 1 }] },
            { id: 4, name: 'Vegetable Soup', servings: 6, ingredients: [{ name: 'carrots', quantity: 3 }, { name: 'potatoes', quantity: 2 }] },
            { id: 5, name: 'Lentil Curry', servings: 4, ingredients: [{ name: 'lentils', quantity: 1 }, { name: 'onions', quantity: 2 }] }
        ];
