...
export class GeminiAI {
  ...
  validateResponse(data, expectedType) {
    ...
    if (expectedType === 'mealplan') {
      ...
      return days.every(
        (day) => data.mealPlan[day] && typeof data.mealPlan[day] === 'string'
      );
    }
  ...
  getOfflineSuggestions(pantry) {
    ...
    return scored.map((s) => ({
      ...
      neededIngredients: s.recipe.ingredients.filter(
        (ing) => !pantryNames.some((p) => ing.includes(p) || p.includes(ing))
      ),
    }));
  }
  ...
  async generateAISuggestions() {
    ...
    const cacheKey = `suggestions-${pantry
      .map((i) => i.name)
      .sort()
      .join(',')}-${JSON.stringify(preferences)}`;
    ...
    try {
      ...
      const allergyInfo =
        preferences.allergy && preferences.allergy !== 'none'
          ? `Allergy: ${preferences.allergy}`
          : 'No allergies';
      const cuisineInfo =
        preferences.cuisine && preferences.cuisine !== 'all'
...
Return ONLY the JSON, no other text.`;
      ...
      this.announce(
        `AI suggestion failed: ${error.message}. Using offline suggestions.`
      );
  ...
  async getSmartMealPlan() {
    ...
    if (pantry.length === 0) {
      this.announce(
        'Add ingredients to your pantry first for AI meal planning'
      );
      return null;
    }
    ...
    const cacheKey = `mealplan-${pantry
      .map((i) => i.name)
      .sort()
      .join(',')}-${people}-${JSON.stringify(preferences)}`;
