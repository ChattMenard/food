...
class AccessibilityManager {
      ...
      mutations.forEach((mutation) => {
        mutation.addedNodes.forEach((node) => {
          if (node.nodeType === Node.ELEMENT_NODE) {
            this.addFocusabilityToNewElements(node);
          }
        });
      });
    });
  ...
  addFocusabilityToNewElements(element) {
    ...
    element.querySelectorAll('button, [onclick], [role="button"], a, input, select, textarea').forEach(child => {
  ...
  addSkipLinks() {
    ...
    skipLinkAnchors.forEach(anchor => {
